...
const allCategories = [
  'All',
  ...Array.from(new Set(graphicDesignsData.map((item) => item.category))),
];

export const GraphicDesigns = () => {
  const { ref } = useSectionInView('Static Designs');
  const [activeCategory, setActiveCategory] = useState('All');

  const filteredItems =
    activeCategory === 'All'
      ? graphicDesignsData
      : graphicDesignsData.filter((item) => item.category === activeCategory);
  ...
  return (
    ...
    >
      ...
      >
        <SectionHeading
          heading="Static Graphic Designs"
          content="Brand identity, print, and document design work."
        />
      </motion.div>
      ...
      <AnimatePresence mode="popLayout">
        ...
        >
          {filteredItems.map((item, index) => (
            <GraphicDesign key={item.title} item={item} index={index} />
          ))}
        </motion.div>
      </AnimatePresence>
    </section>
